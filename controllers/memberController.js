...
const memberController = {
    ...
    getAddMemberPage: function (req, res) {
        ...
        db.find(db.tables.SETTINGS_TABLE, matchesSettingName, [], settingsFields.VALUE, function (result) {
            ...
            console.log(result);
            if (result[0].value == "true" && (req.session.level == undefined || req.session.level == null)) {
                level = 0;
                req.session.level = 0;
            }
            req.session.editId = null;
            if ((level === undefined || level === null) && result[0].value == "false") {
                res.status(401);
                res.render("error", {
                        ...
                        if (result) {
                            ...
                            db.find(db.tables.OBSERVATION_TABLE, observationCondition, null, "*", function (result) {
                                if (result) {
                                    ...
                                    if (parseInt(req.session.level) >= 2) data.backLink = "/member_main_page";
                                    else if (parseInt(req.session.level) == 1) data.backLink = "/main_page";
                                    else {
